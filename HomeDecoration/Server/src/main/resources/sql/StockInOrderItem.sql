with query as (
   ...
   SELECT     ROW_NUMBER() OVER (ORDER BY  a.osNo DESC, a.itm ASC ) AS __rowindex__, a.osNo,a.itm from  [yunfei].[dbo].[T_OrderItemWorkState] as a
...
-- 		   select distinct  PRD_MARK as osNo ,itm   from TF_PSS   WHERE  PS_ID= UPPER ('PC')  AND WH= UPPER ('CP')
	 ) as c on c.osNo=a.osNo collate Chinese_PRC_90_CI_AI and c.itm=a.itm
    where a.workflowstate=VALUE_COMPLETE_STATE  and (a.osNo like :os_no or a.prdNo like :prd_no)


   GROUP BY a.osNo, a.itm
)
...
and  os_dd >'2019-01-01' and (os_no like :os_no or prd_no like :prd_no)
 ...
 --生产方式判断
   ...
   (
       --排厂单
      select  distinct  0 as produceType, SO_NO,EST_ITM ,'' as po_no , sys_date  from  MF_MO    where so_no like upper('%YF%') and so_no like :os_no and  ( so_no like :os_no or mrp_no like  :prd_no ) and mrp_no=MO_NO_ADD
