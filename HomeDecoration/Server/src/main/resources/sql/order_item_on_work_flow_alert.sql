with query as (
   ...
   SELECT     ROW_NUMBER() OVER (ORDER BY  osNo DESC, itm ASC ) AS __rowindex__, osNo,itm from  [yunfei].[dbo].[T_OrderItemWorkState] where (-1=:workflowstate or  workflowstate=:workflowstate)  and ( -1=:workFlowStep or maxWorkFlowStep=:workFlowStep ) and (
...
(
 ...
 (
--9 表示 订单生产中
select osNo,itm,workflowstate,maxWorkFlowStep,maxWorkFlowName, maxWorkFlowCode,workFlowDescribe ,currentOverDueDay,totalLimit ,currentLimitDay,currentAlertDay from  [yunfei].[dbo].[T_OrderItemWorkState] where (-1=:workflowstate or  workflowstate=:workflowstate)  and ( -1=:workFlowStep or maxWorkFlowStep=:workFlowStep ) and (
 ...
 --生产方式判断
   ...
   (
      ...
      select  distinct  0 as produceType, SO_NO,EST_ITM ,'' as po_no , sys_date from  MF_MO    where   bil_Id = upper('MP') and  so_no like upper('%YF%') and so_no like :os_no and mrp_no=MO_NO_ADD
       union
       --外购单
      select distinct 1 as produceType,OTH_NO as so_no,oth_itm1 as est_itm,os_no as po_no, os_dd as sys_date  from  tf_POS   where  os_id=upper('PO') and OTH_NO like upper('%YF%')  and OTH_NO like :os_no and  os_dd >'2019-01-01'
