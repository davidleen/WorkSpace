...
--单位抓取
    left outer join (     select prd_no,idx1, ut from  prdt where   knd='2'

                            ) d  on  B.prd_no=d.prd_no

 left outer join

(select ps_no,os_no,id_no,itm,qty,amt,est_dd from TF_PSS where ps_id=UPPER ('SA')  and est_dd>'2016' and CK_NO =:ck_no ) PS
ON PS.os_no=b.os_no and ps.id_no=b.id_no and ps.itm=b.itm
 left outer join
(select ps_no ,xhfq,xhgh,xhgx from MF_PSS_Z    where ps_id=UPPER ('SA')) PSZ
 ON PS.PS_NO=PSZ.PS_NO
 left outer join
 (select  ps_no, itm,so_zxs,SA_BYXS ,SA_BYTJ from TF_PSS_Z where ps_id=UPPER ('SA') ) TPS
