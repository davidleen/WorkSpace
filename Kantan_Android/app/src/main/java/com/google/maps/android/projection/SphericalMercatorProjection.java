...
public class SphericalMercatorProjection {
    ...
    public Point toPoint(final GeoPoint geoPoint) {
        final double x = geoPoint.getLongitude() / 360 + .5;
        final double siny = Math.sin(Math.toRadians(geoPoint.getLatitude()));
        final double y = 0.5 * Math.log((1 + siny) / (1 - siny)) / -(2 * Math.PI) + .5;

        return new Point(x * mWorldWidth, y * mWorldWidth);
    }

    public GeoPoint toLatLng(com.google.maps.android.geometry.Point point) {
