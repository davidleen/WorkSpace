...
public class KmlLayer extends Layer {
    ...
    public KmlLayer(GoogleMap map, InputStream stream, Context context)
        ...
        KmlRenderer mRenderer = createRenderer(map, context);
        XmlPullParser xmlPullParser = createXmlParser(stream);
        KmlParser parser = new KmlParser(xmlPullParser);
        parser.parseKml();
        stream.close();

        Log.e("TAG","time use in parser:"+(Calendar.getInstance().getTimeInMillis()-time));
    ...
    protected KmlRenderer createRenderer(GoogleMap map, Context context) {
        return new KmlRenderer(map, context);
    }
